...
from apps.strategies.strategies.sma import SMAStrategy  # adjust path as needed
...
def test_calculate_sma_basic(sample_candles):
    ...
    sma = strat.calculate_sma(sample_candles, window=5)
...
def test_calculate_sma_not_enough_data():
    strat = SMAStrategy(short_window=5)
    candles = [{"close": i} for i in range(3)]  # fewer than window
    assert strat.calculate_sma(candles, window=5) == -1


def test_get_list_returns_values(sample_candles):
    strat = SMAStrategy(short_window=5)
    sma_list = strat.get_list_from_candles(sample_candles)
...
def test_generate_sma_json():
    strat = SMAStrategy(short_window=7, long_window=21)
    result = strat.get_json()
    assert result == {"SMAStrategy": {"short_window": 7, "long_window": 21}}


@patch("apps.strategies.strategies.sma.get_binance_ohlcv")
