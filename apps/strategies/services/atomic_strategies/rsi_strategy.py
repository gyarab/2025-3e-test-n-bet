...
class RSIStrategy(IndicatorStrategy):
    ...
    def __init__(
        ...
        """
        if (
            (oversold < 0 or overbought < 0)
            or (oversold >= overbought)
            or (oversold > 50)
            or (overbought > 100)
        ):
            raise ValueError(
                "Invalid RSI thresholds: oversold must be >= 0, overbought must be > oversold, oversold <= 50, overbought <= 100."
            )
    ...
    def from_parametrs(cls, period: int = 14, oversold: int = 30, overbought: int = 70):
        return cls(
            RSIIndicator(period=period), oversold=oversold, overbought=overbought
        )
