...
class SMAStrategy(IndicatorStrategy):
    def __init__(
        ...
        """
        if (
            (short_window < 1 or long_window < 2)
            or (short_window >= long_window)
            or (short_window > 50)
            or (long_window > 300)
        ):
            raise ValueError(
                "Invalid window sizes: short_window must be > 1, long_window must be > short_window, short_window <= 50, long_window <= 300."
            )
    ...
    def from_parametrs(cls, short_window: int = 10, long_window: int = 30):
        return cls(
            SMAIndicator(window=short_window),
    ...
    def get_signal_from_candles(self, candles) -> str:
        ...
        if (
            ...
            or SMA_long_previous is None
            or SMA_short_previous is None
        ):
            return "NOT ENOUGH DATA"
