...
def test_buy_trade_take_profit_hit():
    ...
    trade = TradeEngine(candles, stop_loss_pct=5, take_profit_pct=5, quantity=1000, trade_type=True)
    trade.execute()

    assert trade.status is True or trade.exit_price != 0  # sanity
    assert trade.exit_price == pytest.approx(105)  # take profit
    assert isclose(trade.get_result(), (105 / 100) * 1000 - 1000)
...
def test_buy_trade_stop_loss_hit():
    ...
    assert isclose(trade.get_result(), (95 / 100) * 1000 - 1000)
...
def test_sell_trade_take_profit_hit():
    ...
    trade = TradeEngine(candles, stop_loss_pct=5, take_profit_pct=5, quantity=1000, trade_type=False)
    trade.execute()
    ...
    assert trade.exit_price == pytest.approx(95)
    assert isclose(trade.get_result(), (100 / 95) * 1000 - 1000)
...
def test_sell_trade_stop_loss_hit():
    ...
    assert isclose(trade.get_result(), (100 / 105) * 1000 - 1000)
